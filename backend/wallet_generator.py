...
class WalletGenerator:
    ...
    def generate_entropy(self, bits: int = 128) -> bytes:
        """
        Genera entropía inicial usando secrets para mayor seguridad
        """
        self.entropy = secrets.token_bytes(bits // 8)
        return self.entropy

    def entropy_to_mnemonic(self, entropy: bytes) -> str:
        """
        Convierte la entropía en frase mnemotécnica según BIP39
        """
        # 1. Calcular checksum
        ent = len(entropy) * 8
    ...
    def mnemonic_to_seed(self, mnemonic: str, passphrase: str = "") -> bytes:
        """
        Genera la semilla a partir de la frase mnemotécnica usando PBKDF2
        """
